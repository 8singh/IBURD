...
def create_mask(seg,img, save):
    bb = []
    for i in range (0,len(seg),2):
        bb.append([seg[i],seg[i+1]])
    area = np.array(bb)
    filled = np.zeros(img.shape)
    filled = cv2.fillPoly(filled, pts = [area], color = [255,255,255])
    cv2.imwrite(save+"mask.png", filled)
    return filled

def blur(image, size=60):
    (h, w) = image.shape
...
def poisson_blending(source,target,mask,offset):
    ...
    mask = cv2.cvtColor(cv2.warpAffine(mask, M, (x_range, y_range)), cv2.COLOR_BGR2GRAY)
...
def rotate_seg_resize(seg, angle, size, factor):
    origin = (0,0)
    new_seg = []
    for i in range (0,len(seg),2):
        x_new,y_new = rotate([np.round(seg[i]*factor),np.round(seg[i+1]*factor)], np.radians(angle), origin)
        new_seg.append(x_new)
        new_seg.append(y_new)
    return new_seg
...
def seg_to_box(seg):
    ...
    return [x_min,y_min,xmax-xmin,ymax-ymin] 
